...
public class DefaultShaderInterface implements ChunkShaderInterface {
    ...
    public DefaultShaderInterface(ShaderBindingContext context, ChunkShaderOptions options) {
        ...
        this.uniformTextureMatrix = context.bindUniformOptional("u_TextureMat", GlUniformMatrix4f::new);
        ...
        if (pass != DefaultTerrainRenderPasses.SKY_BLOCK) {
            ...
            ((GlProgram) context).bind();
            GL46C.glUniform1i(GlStateManager._glGetUniformLocation(((GlProgram) context).handle(), "u_blockTex"), 0);
            GL46C.glUniform1i(GlStateManager._glGetUniformLocation(((GlProgram) context).handle(), "u_skyTex"), 1);
            GL46C.glUniform1i(GlStateManager._glGetUniformLocation(((GlProgram) context).handle(), "u_glintTex"), 2);
        }
    ...
    public void setupState() {
        ...
        if (this.uniformTextureMatrix != null) {
            this.uniformTextureMatrix.set(RenderSystem.getTextureMatrix());
        }
    ...
    private void bindTextureC(int i) {
        GlTexture tex = (GlTexture) RenderSystem.getShaderTexture(i);
        GlStateManager._activeTexture(GL32C.GL_TEXTURE0 + i);
